...
check_keyring_entry_exists <- function(service_name, user_name) {
  tryCatch(
    ...
    error = function(cond) {
      ...
      missing_element <- stringr::str_detect(
...
#'     "secret_one" = keyring_block_list_one,
...
get_username_from_block <- function(keyring_block) {
  return(keyring::key_get(
    service = keyring_block$service,
    username = "username"
  ))
}
...
check_table_entries_available <- function(keyring_table) {
  ...
  service <- NULL
  username <- NULL
  entry_exists <- if (is.null(entry_exists)) 1 else 0
  ...
  adjusted_keyring_table <- keyring_table |>
    dplyr::rowwise() |>
    dplyr::mutate(
      entry_exists = check_keyring_entry_exists(
        !!service_var,
        !!username_var
      )
    ) |>
...
raise_missing_entry_message <- function(keyring_table) {
  ...
  for (row_ind in seq_len(nrow(adjusted_keyring_table))) {
